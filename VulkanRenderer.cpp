...
void VulkanRendererApp::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
	// vk image creation
	...
	imageInfo.format = format; // same format as pixels in the buffer
	imageInfo.tiling = tiling; // either optimal or row-major layout, cannot be changed later
	imageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; // either that or preinitialized, which will preserve texels during first transition
...
void VulkanRendererApp::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void VulkanRendererApp::createTextureSampler()
{
	VkSamplerCreateInfo samplerInfo{};
	samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
	samplerInfo.magFilter = VK_FILTER_LINEAR; // how to interpolate texels
	...
	samplerInfo.unnormalizedCoordinates = VK_FALSE; // range 0,1 True would be the x,y range of texture
